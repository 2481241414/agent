...
# --- 文件路径配置 ---
INPUT_INFERENCE_PATH = "/home/workspace/lgq/distill/data/20250721/fc/qwen2.5_7b_FC能力验证_inference_results.json"
OUTPUT_EVALUATION_DETAILS_PATH = "/home/workspace/lgq/distill/data/20250721/fc/qwen2.5_7b_FC能力验证_evaluation_tag.csv"
...
# --- MODIFIED: print_summary_report 函数现在接受一个标题参数 ---
def print_summary_report(detailed_results, report_title="模型综合"):
    total_samples = len(detailed_results)
    if total_samples == 0: 
        print(f"\n" + "="*20 + f" {report_title} 评估报告 " + "="*20)
        print("该类别下无样本。")
        print("="*58)
        return
...
# --- MODIFIED: main 函数现在会按 tag 分组打印报告 ---
def main():
    try:
    ...
    # 4. 按 tag 分组并打印报告
    #   a. 获取所有唯一的 tag
    all_tags = sorted(list(set(r['标签(tag)'] for r in detailed_results if r['标签(tag)'])))
    
    #   b. 遍历每个 tag 并生成报告
    for tag in all_tags:
        results_for_tag = [r for r in detailed_results if r['标签(tag)'] == tag]
