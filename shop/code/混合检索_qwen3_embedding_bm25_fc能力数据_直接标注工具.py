...
class BM25Retriever:
    ...
    def __init__(self, data_df: pd.DataFrame, all_tools_definitions: list, k1=1.5, b=0.75):
        ...
        # 注意: BM25的语料库构建依赖于有'指令'和'ground_truth_tool'列的原始数据
    ...
    def _build_keyword_rich_corpus(self, data_df: pd.DataFrame) -> list:
        ...
        for _, row in data_df.iterrows():
            ...
            if 'ground_truth_tool' in row and isinstance(row.get('ground_truth_tool'), list) and row['ground_truth_tool']:
                 # 确保 '指令' 列存在
                if '指令' in row and pd.notna(row['指令']):
...
class InstructionSearcher:
    ...
    def _build_mappings(self, data_df: pd.DataFrame):
        ...
        # 确保使用有 '指令' 列的数据进行映射构建
        data_with_instructions = data_df.dropna(subset=['指令', 'ground_truth_tool'])

        for _, row in data_with_instructions.drop_duplicates(subset=['指令'], keep='last').iterrows():
...
def evaluate_recall_system(data_df, all_bm25_scores, all_semantic_scores, all_tools_definitions, alpha, k_values, full_report=False):
    ...
    for i, (_, row) in enumerate(data_df.iterrows()):
        ...
        if full_report:
            ...
            original_query_text = row.get('query', query_text) # 兼容旧格式
...
def main():
    ...
    try:
        # 需要 '指令' 和 'ground_truth_tool' 来构建召回器
        base_required_cols = ['指令', 'ground_truth_tool']
        base_df = pd.read_csv(base_data_file_path, usecols=lambda c: c in base_required_cols or c in ['query', 'plan（在xx中做什么）']).dropna(subset=base_required_cols).reset_index(drop=True)
        ...
        print(f"基础数据加载完成: 共 {len(base_df)} 条。\n")
    ...
    # 为了演示，我们先对原始数据集进行评估
    # --- 3. 对原始数据集进行评估 ---
    print("\n--- 步骤 3 & 4 & 5: 对原始数据集进行评估流程 ---")
    
    # 使用 'plan（在xx中做什么）' 作为查询
    eval_df = base_df.dropna(subset=['plan（在xx中做什么）']).copy()

    print("--> 正在为原始数据集计算召回分数...")
    all_bm25_scores = [bm25_retriever.retrieve_scores(row['plan（在xx中做什么）']) for _, row in tqdm(eval_df.iterrows(), total=len(eval_df), desc="计算BM25分数")]
    all_semantic_scores = [instruction_searcher.retrieve_scores(row['plan（在xx中做什么）']) for _, row in tqdm(eval_df.iterrows(), total=len(eval_df), desc="计算语义分数")]
    
    print("--> 正在为原始数据集进行Alpha值网格搜索...")
    alpha_range = np.linspace(0, 1, 21) # 减少步数以加快演示
    best_alpha = -1
    best_score = -1
...
# 【新增】区域 7: 新格式数据集评测函数
# ==============================================================================
def run_evaluation_on_new_dataset(test_file_path: str, bm25_retriever: BM25Retriever, instruction_searcher: InstructionSearcher, all_tools_definitions: list, alpha: float, k_values: list):
    """
    加载、筛选、处理并评测一个新格式的数据集。

    Args:
        test_file_path (str): 新格式测试数据文件的路径。
        bm25_retriever (BM25Retriever): 训练好的BM25召回器。
        instruction_searcher (InstructionSearcher): 训练好的意图搜索器。
        all_tools_definitions (list): 全局工具定义列表。
        alpha (float): 用于融合分数的最佳权重。
        k_values (list): 用于评测的K值列表 (例如 [1, 3, 5])。
    """
    print(f"\n\n--- 步骤 9: 开始针对新格式数据集进行专项评测 ---")
    ...
    try:
        # 定义新数据集中需要的列
        required_cols = ['tag', 'plan_out', '指令对应tool']
        test_df = pd.read_csv(test_file_path, usecols=required_cols)
        
        # 定义要筛选的tag
        tags_to_include = ['单指令单工具-常规', '单指令单工具-多槽位', '单指令单工具-工具多传']
        
        filtered_df = test_df[test_df['tag'].isin(tags_to_include)].dropna(subset=['plan_out', '指令对应tool']).reset_index(drop=True)
        
        if len(filtered_df) == 0:
            print("错误: 根据tag筛选后，没有可用于评测的数据。请检查CSV文件内容和tag值。")
            return
            
        print(f"数据加载完成。根据tag筛选后，共获得 {len(filtered_df)} 条测试数据。")
    ...
    # --- 2. 预处理数据 (适配ground_truth格式) ---
    def parse_new_gt(tool_string: str) -> list:
        """将字符串格式的工具名转换为评测函数所需的列表字典格式。"""
        if isinstance(tool_string, str) and tool_string.strip():
            # 清理字符串，并只取函数签名部分
            cleaned_name = tool_string.strip()
            gt_func_name = cleaned_name.split('(')[0]
            
            # 从全局定义中找到最匹配的官方工具名，以保证参数部分一致
            for tool_def in all_tools_definitions:
                if tool_def['name'].startswith(gt_func_name + '('):
                    return [{'name': tool_def['name']}]
            # 如果没找到完全匹配的，则使用清理后的名字（容错）
            return [{'name': cleaned_name}]
        return []

    filtered_df['ground_truth_tool'] = filtered_df['指令对应tool'].apply(parse_new_gt)
