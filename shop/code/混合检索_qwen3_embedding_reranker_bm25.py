...
try:
    ...
    print(f"错误: 缺少必要的库 -> {e}")
    print("请在终端运行: pip install faiss-cpu torch sentence-transformers transformers rank_bm25 scikit-learn pandas tqdm jieba")
    exit()
...
class InstructionSearcher:
    ...
    def __init__(self, data_df: pd.DataFrame, all_tools_definitions: list, model_name: str):
        ...
        print(f"--- [意图通路] 正在加载语义召回模型: {model_name} ---")
        ...
        print("--- [意图通路] Embedding+Faiss召回器准备就绪 ---\n")
...
# ==============================================================================
# 区域 2.1: QwenReranker 精排器类 (已修复显存问题)
# ==============================================================================
class QwenReranker:
    ...
    def __init__(self, model_name: str, instruction: str):
        ...
        self.max_length = 128  # **仅用于截断**，防止输入过长，不影响填充
...
def evaluate_with_reranker(
    ...
    iterator = tqdm(data_df.iterrows(), total=len(data_df), desc="评测中(召回+精排)")
    for i, row in iterator:
        # Reranker query 使用 'plan'
...
def main():
    ...
    EMBEDDING_MODEL_NAME = '/home/workspace/lgq/shop/model/Qwen3-Embedding-0.6B'
    RERANKER_MODEL_NAME = '/home/workspace/lgq/shop/model/Qwen3-Reranker-0.6B' # 本地路径
    ...
    instruction_searcher = InstructionSearcher(data_df, all_tools_definitions, model_name=EMBEDDING_MODEL_NAME)
    reranker = QwenReranker(model_name=RERANKER_MODEL_NAME, instruction=RERANKER_INSTRUCTION)
    
    # --- 新增: 为Reranker准备基于 '指令' 的文档 ---
    print("--- 步骤 2.5: 为Reranker准备基于 '指令' 的文档集 ---")
    ...
    for tool in all_tools_definitions:
        ...
        if instructions:
            ...
            # 如果某个工具在数据集中没有对应的指令，则回退到使用其描述
            reranker_docs_by_index.append(tool['description'])
    print("--- Reranker文档集准备完成 ---\n")
    
    # --- 3. Alpha值网格搜索 ---
    print("\n--- 步骤 3: 开始进行召回阶段的 Alpha 值网格搜索 ---")
    best_alpha = 0.1 # 原有值
    print(f"注意: 为快速演示，已跳过Alpha网格搜索，直接使用召回阶段的 best_alpha = {best_alpha}")

    # --- 4. 使用最佳Alpha进行最终的、完整的评测 ---
    print(f"\n--- 步骤 4: 使用召回Alpha={best_alpha:.2f}和Rerank_Top_N={RERANK_TOP_N}进行最终完整评测 ---")
    # --- 修改点 3：传递新的 reranker_docs_by_index ---
    results, error_cases = evaluate_with_reranker(
        data_df, 
        bm25_retriever, 
        instruction_searcher,
        reranker,
        all_tools_definitions,
        reranker_docs_by_index, # 传递新文档
        alpha=best_alpha, 
        k_values=K_VALUES,
    ...
    print(f"召回+精排模型 (BM25+Embedding Recall, Alpha={best_alpha:.2f}; Qwen Reranker, Top-N={RERANK_TOP_N}) 评测结果:")
    print("-" * 80)
