...
def set_seed(seed: int = 42):
    """
    设置所有相关的随机种子，并配置PyTorch以使用确定性算法。
    
    Args:
        seed (int): 您选择的种子数。
    """
    # 1. 设置 Python、NumPy 和 PyTorch 的 CPU 种子
    # random.seed(seed) # 如果您使用了 random 库
    np.random.seed(seed)
    torch.manual_seed(seed)
    
    # 2. 如果使用 GPU，也为所有 GPU 设置种子
    if torch.cuda.is_available():
        torch.cuda.manual_seed(seed)
        torch.cuda.manual_seed_all(seed) # 为所有GPU设置种子
    
    # 3. 配置 PyTorch 的 cuDNN 后端
    #    这是保证 GPU 运算结果一致性的关键步骤
    #    设置为 True，cuDNN 会使用确定性算法，可能会牺牲一点性能
    torch.backends.cudnn.deterministic = True
    #    禁用 cuDNN 的基准测试模式，该模式会为不同输入尺寸选择不同算法
    torch.backends.cudnn.benchmark = False
    
    print(f"--- [系统] 所有随机种子已固定为: {seed} ---")
    print(f"--- [系统] PyTorch cuDNN 已配置为确定性模式 ---")
...
def get_exact_tool_definitions():
    ...
    full_tools = [
        {"name": "search_goods(app, search_info_slot, page_type, filter_detail_slot, order_type)", "description": "在app程序中依据名称搜索商品，可以指定具体在哪一个子页面进行搜索，搜索结果的筛选条件和排序方式"},
        {"name": "search_stores(app, search_info_slot, filter_detail_slot, order_type)", "description": "在app程序中依据名称搜索店铺，可以使用筛选器限制搜索结果，也可以指定搜索结果的排序方式"},
        ...
        {"name": "search_cart_content(app, search_info_slot)", "description": "在app应用程序中的购物车/采购车（阿里巴巴的叫法）查找商品"},
        ...
        {"name": "open_goods_page(app, search_info_slot, store_name_slot, page_type)", "description": "通过商品名称找到并打开其详细信息页面，可以指定子页面包括：评论、参数等"},
        ...
        {"name": "open_special_page(app, page_type)", "description": "打开特殊页面，例如国家补贴、政府补贴等活动页面"},
        ...
        {"name": "add_into_favorite_stores(app, search_info_slot)", "description": "在app程序中按照店铺名搜索店铺，并将其添加到店铺藏夹中"},
        {"name": "delete_favorite_goods(app, search_info_slot, filter_info)", "description": "在app程序的商品收藏夹中搜索指定商品并将其删除"},
        ...
        {"name": "open_orders_bought(app, order_status, search_info_slot, filter_detail_slot)", "description": "在app应用程序中查看买入的指定状态的订单列表，例如待付款、待收货、待评价等。"},
        {"name": "apply_after_sales(app, search_info_slot, after_sales_type_slot, reason_slot)", "description": "在app应用程序中为指定搜索内容的订单申请售后服务，如：退款、退货、换货、价保、维修等"},
        {"name": "open_logistics_receive(app, filter_type)", "description": "根据用户指定的物流状态查看已购买商品列表。"},
        ...
        {"name": "open_customer_service(app, order_slot, store_slot)", "description": "在app应用程序中联系官方客服，或联系指定订单的店铺客服，或联系指定店铺的客服"},
...
# 区域 6: 主程序
# ==============================================================================
def load_data_from_jsonl(file_path: str, tool_definitions: list) -> pd.DataFrame:
    """
    从 JSONL 文件加载评测数据，并进行预处理。
    
    Args:
        file_path (str): .jsonl 文件的路径。
        tool_definitions (list): 包含所有工具定义的列表，用于反向查找。

    Returns:
        pd.DataFrame: 包含 'query' 和 'ground_truth_tool' 列的 DataFrame。
    """
    # 创建一个从 description 到 标准化 tool 定义的映射
    desc_to_tool_map = {tool['description']: tool for tool in tool_definitions}
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                try:
                    ...
                    response_desc = record.get('response') # 这是工具的描述
                    
                    if not query or not response_desc:
                        continue
                    
                    # 根据 response (description) 找到对应的工具定义
                    ground_truth_tool_def = desc_to_tool_map.get(response_desc)
                    
                    if ground_truth_tool_def:
                        # 将找到的工具定义包装成评测函数期望的列表格式
                        # 评测函数期望的是一个工具列表，即使只有一个
                        data_records.append({
                            'query': query,
                            'ground_truth_tool': [ground_truth_tool_def] 
                        })
                    else:
                        print(f"警告: 无法为描述找到对应的工具定义 -> '{response_desc}'")
                        
                except json.JSONDecodeError:
                    print(f"警告: 跳过无法解析为JSON的行 -> {line.strip()}")
...
async def main():
    ...
    # 请确保这里的模型路径是正确的
    VLLM_SERVED_MODEL_NAME = "/home/workspace/lgq/ms-swift/output/recall-qwen3-0.6-embedding/v0-20250815-045723/checkpoint-480"
    API_CONCURRENCY_LIMIT = 200
    
    # --- 修改点 1: 将测试数据文件路径改为 .jsonl 文件 ---
    annotated_data_file_path = '/home/workspace/lgq/ms-swift/data/endtoend_data.jsonl'
    # annotated_data_file_path = '/home/workspace/lgq/ms-swift/data/single_task_data.jsonl'
    # annotated_data_file_path = '/home/workspace/lgq/ms-swift/data/zhiyu_data.jsonl'
    
    K_VALUES = [1, 2, 3, 5, 10]
    NUM_ERROR_EXAMPLES_TO_PRINT = 5
    # --- 修改点 2: 简化输出路径，因为不再有子集 ---
    output_file_path = f'/home/workspace/lgq/shop/data/evaluate/endtoend/recall_results_query_vs_desc_sft_{MODE}.csv'
    # output_file_path = f'/home/workspace/lgq/shop/data/evaluate/single_task/recall_results_query_vs_desc_sft_{MODE}.csv'
    # output_file_path = f'/home/workspace/lgq/shop/data/evaluate/zhiyu/recall_results_query_vs_desc_sft_{MODE}.csv'

    # --- 1. 初始化工具定义 (需要先于数据加载) ---
    all_tools_definitions = get_exact_tool_definitions()

    # --- 2. 加载和预处理数据集 (从JSONL) ---
    print("--- 步骤 1: 从 JSONL 文件加载和预处理数据集 ---")
    data_df = load_data_from_jsonl(annotated_data_file_path, all_tools_definitions)
    
    if data_df.empty:
        print("错误: 加载数据后 DataFrame 为空，程序终止。")
        return
    ...
    # --- 5. 运行评测 (直接在整个数据集上) ---
    # --- 修改点 3: 移除子集循环，直接调用一次评测函数 ---
    print(f"\n\n{'='*30}\n--- 开始对整体数据集 (共 {len(data_df)} 条) 进行评测 ---\n{'='*30}")
    
    # best_alpha, best_score = -1, -1
    # for alpha in tqdm(np.linspace(0, 1, 51), desc="Alpha网格搜索"):
    #     score = evaluate_recall_system(data_df, all_bm25_scores, all_semantic_scores, all_tools_definitions, alpha, K_VALUES)
    #     if score > best_score:
    #         best_score, best_alpha = score, alpha
    
    best_alpha = 0
    best_score = 0
    print(f"\n--- 最佳Alpha值: {best_alpha:.2f} (最高Recall@1: {best_score:.4f}) ---")
    ...
    print(f"\n--- Top-1 错误案例分析 (共 {len(error_cases)} 个错误) ---")
    if not error_cases:
