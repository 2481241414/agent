...
# --- 文件路径配置 ---
# 请将这里更新为您新数据文件的路径
INPUT_INFERENCE_PATH = "/home/workspace/nl/FC/data/output/toolcall/2025.07.22_Qwen2.5-7B-Agent-Instruct/fc_evaluation_with_response.json" 
# 输出文件的路径可以自定义
OUTPUT_EVALUATION_DETAILS_PATH = "/home/workspace/lgq/distill/data/20250730/fc/yunke0722_evaluation_details_with_tags.csv"
...
def parse_dsl_string(dsl_str: str):
    ...
    [MODIFIED] 更新正则表达式以兼容带引号和不带引号的参数值。
    """
    ...
    try:
        ...
        # 新的正则表达式: (\w+)\s*=\s*(?:"(.*?)"|([^,)]+))
        # 它可以匹配 key="value" (捕获组2) 和 key=value (捕获组3)
        param_matches = re.findall(r'(\w+)\s*=\s*(?:"(.*?)"|([^,)]+))', params_str)
        # 将匹配结果转换为字典，取非空的捕获组作为值
...
def parse_tools_from_prompt(user_prompt: str):
    """
    从Prompt中解析工具定义。
    [MODIFIED] 使用 ast.literal_eval 并调整解析逻辑以适应新的工具格式。
    """
    tools_def = {}
    try:
...
def calculate_prf1(tp, fp, fn):
    """计算精确率、召回率和F1值"""
    precision = safe_div(tp, tp + fp, to_percent=False) # 改为默认输出小数
    recall = safe_div(tp, tp + fn, to_percent=False)
    f1 = (2 * precision * recall) / (precision + recall) if (precision + recall) > 0 else 0
    # 在报告中格式化为百分比，这里返回原始值便于聚合
...
def run_evaluation_and_get_details(inference_results):
    ...
    for result in inference_results:
        ...
        # 注意：这里的完整匹配可能因为新数据中不带引号而失败，这是预期行为
...
def main():
    ...
    for item in new_format_data:
        ...
        adapted_item = {
            ...
            "model_output": item.get("predict"),
            "delay_seconds": -1.0,  # 新数据中无此字段，设为默认值
            "tag": item.get("tag") 
        }
        inference_results_adapted.append(adapted_item)
    
    print(f"已将 {len(inference_results_adapted)} 条数据成功适配，开始评估...")
